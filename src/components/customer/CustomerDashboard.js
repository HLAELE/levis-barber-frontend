...
function CustomerDashboard({ user, onLogout }) {
    ...
    const bookAppointment = async () => {
        if (!booking.custom_service || !booking.appointment_date || !booking.appointment_time || !booking.amount) { alert('Please fill all fields'); return; }
        try {
        ...
        } catch (error) { console.error(error); alert('Failed to book'); }
    };

    const downloadReceipt = async (appointmentId) => {
        try {
            const response = await axios.get(`${API_URL}/customer/download-receipt/${appointmentId}`, { headers: { Authorization: `Bearer ${token}` }, responseType: 'blob' });
    ...
    return (
        <div className="dashboard-container">
            <div className="sidebar">
                ...
                <div className="sidebar-footer"><div>👤 {user?.full_name}</div><button className="logout-btn" onClick={onLogout}>Logout</button></div>
            </div>
            <div className="main-content">
                {activeTab === 'book' && (
                    <div>
                        <h2>Book Appointment</h2>
                        <textarea className="input-field" placeholder="Describe your hair service..." rows="3" value={booking.custom_service} onChange={(e) => setBooking({ ...booking, custom_service: e.target.value })} />
                        ...
                        <select className="input-field" value={booking.payment_method} onChange={(e) => setBooking({ ...booking, payment_method: e.target.value })}><option>CASH</option><option>CARD</option><option>MOBILE</option></select>
                ...
                {activeTab === 'appointments' && (
                    <div>
                        ...
                        <table className="data-table"><thead><tr><th>Service</th><th>Barber</th><th>Date</th><th>Time</th><th>Status</th><th>Payment</th><th>Receipt</th></tr></thead>
                        <tbody>{myAppointments.map(a => (<tr key={a.appointment_id}><td>{a.service_description || a.custom_service}</td><td>{a.barber_name || 'Pending'}</td><td>{a.appointment_date}</td><td>{a.appointment_time}</td><td><span className={`status-${(a.status || 'PENDING').toLowerCase()}`}>{a.status || 'PENDING'}</span></td><td><span className={a.payment_status === 'PAID' ? 'paid' : 'unpaid'}>{a.payment_status || 'UNPAID'}</span></td><td><button className="btn-success" onClick={() => downloadReceipt(a.appointment_id)}>📥 Receipt</button></td></tr>))}</tbody></table>
                    </div>
                )}
